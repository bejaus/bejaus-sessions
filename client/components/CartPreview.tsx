...
export function CartPreview({ children }: CartPreviewProps) {
  ...
  const handleUpdateQuantity = (productId: string, newQuantity: number, selectedSize?: string) => {
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <PopoverContent className="w-80 p-4" align="end">
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-forest-green mb-2">Tu carrito está vacío</h3>
            <p className="text-forest-green/70 text-sm mb-4">¡Añade algunos productos para empezar!</p>
      ...
      <PopoverContent className="w-96 p-0" align="end">
        <div className="p-4 border-b border-forest-green/10">
          <h3 className="text-lg font-semibold text-forest-green">
            Tu carrito ({cart.itemCount} {cart.itemCount === 1 ? 'artículo' : 'artículos'})
          </h3>
        </div>
        
        <div className="max-h-96 overflow-y-auto">
          {cart.items.map((item, index) => (
            <div key={`${item.product.id}-${item.selectedSize || 'no-size'}`} className="p-4 border-b border-forest-green/5 last:border-b-0">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.selectedSize && (
                    <p className="text-xs text-forest-green/70 mb-1">Talla: {item.selectedSize}</p>
                  )}
                  {item.selectedColor && (
                    <p className="text-xs text-forest-green/70 mb-2">Color: {item.selectedColor}</p>
                  )}
        ...
        <div className="p-4 bg-forest-green/5 border-t border-forest-green/10">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-lg font-bold text-forest-green">{cart.total.toFixed(2)}€</span>
          </div>
          
          {cart.total < 50 && (
            <p className="text-xs text-terracotta mb-3">
              Añade {(50 - cart.total).toFixed(2)}€ más para conseguir envío gratuito
            </p>
          )}
